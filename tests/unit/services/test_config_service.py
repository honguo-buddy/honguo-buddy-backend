...
async def test_dynamic_config_manager_seed_load_and_get():
    ...
    assert any(isinstance(item, SysConfig) and item.config_key == "MAX_OPEN_BUY_POSTS_PER_USER" for item in seed_db.added)
    ...
    load_db = build_db(
        execute_side_effect=[
            FakeResult(
                items=[
                    SysConfig(config_key="MAX_OPEN_BUY_POSTS_PER_USER", config_value="15", config_type="int", description="x"),
    ...
    assert manager.get("MAX_OPEN_BUY_POSTS_PER_USER") == 15
...
@pytest.mark.asyncio
async def test_dynamic_config_manager_seed_inserts_missing_defaults_when_table_not_empty():
    manager = DynamicConfigManager()
    ...
    class PartialDB:
        def __init__(self):
            ...
            self.execute = AsyncMock(
                side_effect=[
                    FakeResult(
                        items=["USER_INITIAL_CREDIT_SCORE"]
                    )
                ]
            )
    ...
    partial_db = PartialDB()
    await manager.seed_defaults_if_empty(partial_db)

    added_keys = {item.config_key for item in partial_db.added}
    assert "USER_INITIAL_CREDIT_SCORE" not in added_keys
    assert "MAX_OPEN_BUY_POSTS_PER_USER" in added_keys
    assert "MAX_OPEN_SELL_POSTS_PER_USER" in added_keys
    assert "MAX_OPEN_GOODS_PER_USER" in added_keys
    partial_db.commit.assert_awaited_once()
...
async def test_config_service_list_and_get_detail():
    ...
    assert any(item.config_key == "MAX_OPEN_BUY_POSTS_PER_USER" for item in config_list)
    assert any(item.config_key == "HISTORY_MAX_SIZE" for item in config_list)

    detail = await ConfigService.get_config_by_key(db, config_key="MAX_OPEN_BUY_POSTS_PER_USER")
    assert detail.config_key == "MAX_OPEN_BUY_POSTS_PER_USER"
...
def test_settings_dynamic_proxy_override():
    manager = DynamicConfigManager()
    manager._cache["MAX_OPEN_BUY_POSTS_PER_USER"] = 33
    assert manager.get("MAX_OPEN_BUY_POSTS_PER_USER") == 33
