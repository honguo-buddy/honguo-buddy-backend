...
from app.core.datetime_utils import get_now, get_now_naive
...
def create_access_token(data: dict, expires_delta: timedelta = None):
    to_encode = data.copy()
    # 使用带时区的时间，避免与 jose 的 UTC 验证冲突
    expire = get_now() + (expires_delta or timedelta(minutes=settings.TOKEN_EXPIRE_TIME))
...
def generate_email_verify_token(email: str):
    # 使用带时区的时间（北京时区），确保 jwt 库校验 exp 时行为一致
    expire = get_now() + timedelta(minutes=settings.EMAIL_VERIFY_EXPIRE_MINUTES)
