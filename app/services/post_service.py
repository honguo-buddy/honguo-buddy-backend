...
class PostService:
    ...
    @staticmethod
    async def _hydrate_posts_avatar(db: AsyncSession, posts: List[Post]) -> None:
        """核心对齐：建立帖子域应用层批量头像 URL 灌水中心，彻底绞杀 N+1 问题"""
        if not posts:
            return
            
        # 1. 批量收集这批帖子发布者的 avatar_id
        avatar_ids = []
        for p in posts:
            if p.user and getattr(p.user, "avatar_id", None):
                avatar_ids.append(p.user.avatar_id)
                
        # 2. 集中抛出一发高效的 IN 查询，把所有头像的真实相对路径一次性拉出来
        avatar_url_map = {}
        if avatar_ids:
        ...
        # 3. 强行突破 ORM 限制，在内存中动态为 user 注入 avatar 属性，供 Pydantic 响应 Schema 畅快读取
        for p in posts:
            if p.user:
                p_avatar_id = getattr(p.user, "avatar_id", None)
                p.user.avatar = avatar_url_map.get(p_avatar_id) if p_avatar_id else None
