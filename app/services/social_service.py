    ...
    async def toggle_favorite(db: AsyncSession, user_id: int, target_type: str, target_id: int) -> dict[str, Any]:
        ...
        if normalized_target == FavoriteTargetType.POST:
            ...
            if not target_obj or target_obj.is_deleted:
                raise ResourceHTTPException(
                    message="帖子不存在或已被删除",
                    error_code=settings.DATA_GET_FAILED_CODE,
                )
        else:
                ...
                items.append(
                    {
                        ...
                        "target_status": "SOLD" if goods_item and getattr(goods_item, "is_sold", False) else "AVAILABLE",
                        "is_effective": bool(goods_item and not goods_item.is_deleted and not getattr(goods_item, "is_sold", False)),
                        "is_full": getattr(goods_item, "is_sold", False) if goods_item else False,
                        "create_time": int(fav.create_time.timestamp() * 1000) if fav.create_time else 0,
